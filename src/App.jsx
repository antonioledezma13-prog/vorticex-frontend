...
function AnalyticsView({ user }) {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 animate-fadeIn max-w-7xl mx-auto">
      {/* ENCABEZADO */}
        ...
        <>
          {activeSubTab === 'vortex' && vortexData && (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-fadeIn">
              <div className="bg-neutral-900 border border-neutral-800 p-6 rounded-2xl flex flex-col items-center justify-center text-center relative overflow-hidden shadow-[0_4px_20px_rgba(16,185,129,0.02)]">
              ...
              <div className="bg-neutral-900 border border-neutral-800 p-6 rounded-2xl flex flex-col items-center justify-center text-center relative overflow-hidden shadow-[0_4px_20px_rgba(234,179,8,0.02)]">
              ...
              <div className="bg-neutral-900 border border-neutral-800 p-6 rounded-2xl flex flex-col items-center justify-center text-center relative overflow-hidden shadow-[0_4px_20px_rgba(239,68,68,0.02)]">
                  ...
                  {vortexData.roiProgress?.map((item, idx) => (
                    <div key={idx} className="flex items-center justify-between p-3.5 bg-neutral-950/60 rounded-xl border border-neutral-850">
                      <div className="flex items-center gap-3">
                        <span className={`w-2.5 h-2.5 rounded-full ${item.resultado === 'Ganado' ? 'bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]' : 'bg-red-500'}`} />
                        <div>
                        ...
                        <td className="p-4 pr-6">
                          <div className="flex gap-1.5">
                            {tipster.racha.map((r, rIdx) => (
                              <span key={rIdx} className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-black ${r === 'W' ? 'bg-emerald-950 border border-emerald-500 text-emerald-400 shadow-[0_0_6px_rgba(16,185,129,0.3)]' : 'bg-red-950 border border-red-900 text-red-400'}`}>{r}</span>
                            ))}
                          </div>
                        </td>
                      </tr>
        ...
        <nav className="space-y-1">
          {items.map(item => (
            <button
              ...
              className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl font-bold text-sm tracking-wide uppercase transition-all bg-transparent border-none cursor-pointer text-left ${
                activePage === item.id 
                  ? 'bg-gradient-to-r from-red-600 to-yellow-500 text-neutral-950 shadow-lg shadow-red-500/10 font-black' 
...
function Topbar({ activeTab, onTabChange, onOpenAuth, onNavigate }) {
  ...
  return (
    <header className="h-16 border-b border-neutral-800 bg-neutral-900/60 backdrop-blur-md flex items-center justify-between px-6 sticky top-0 z-20 font-sans">
      ...
      <div>
        {user ? (
          <button onClick={logout} className="text-xs font-bold uppercase tracking-wider bg-neutral-800 border border-neutral-700 hover:bg-neutral-700 px-4 py-2 rounded-xl text-neutral-300 transition-all cursor-pointer">Cerrar Sesión</button>
        ) : (
          <button onClick={onOpenAuth} className="text-xs font-black uppercase tracking-wider bg-gradient-to-r from-red-600 to-yellow-500 text-neutral-950 px-4 py-2 rounded-xl transition-all hover:scale-[1.02] shadow-[0_0_15px_rgba(239,68,68,0.2)] cursor-pointer">Iniciar Sesión</button>
        )}
      </div>
    </header>
  );
    ...
    <div className="p-6 space-y-8 max-w-7xl mx-auto font-sans">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {sampleEvents.map(event => (
          <div key={event.id} className="bg-neutral-900 border border-neutral-850 rounded-2xl p-5 relative overflow-hidden flex flex-col justify-between hover:border-neutral-700 hover:shadow-[0_4px_30px_rgba(239,68,68,0.03)] transition-all">
            <div className="space-y-4">
              ...
              <div className="flex gap-1.5 justify-center py-2 border-t border-neutral-850/40 mt-3">
                {event.formHome.map((r, idx) => (
                  <span key={idx} className={`w-5 h-5 rounded-full flex items-center justify-center text-[9px] font-black ${r === 'W' ? 'bg-emerald-950/60 border border-emerald-500/20 text-emerald-400 shadow-[0_0_6px_rgba(16,185,129,0.2)]' : r === 'D' ? 'bg-yellow-950/60 border border-yellow-500/20 text-yellow-400 shadow-[0_0_6px_rgba(234,179,8,0.2)]' : 'bg-red-950/60 border border-red-500/20 text-red-400 shadow-[0_0_6px_rgba(239,68,68,0.2)]'}`}>{r}</span>
                ))}
              </div>
            </div>
...
function Plans({ onOpenAuth }) {
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-8 text-center animate-fadeIn font-sans">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-2xl mx-auto mt-6">
        <div className="bg-neutral-900 border border-neutral-800 p-8 rounded-2xl space-y-6 flex flex-col justify-between">
          <div>
            ...
            <ul className="text-sm text-neutral-400 text-left space-y-3 mt-6 border-t border-neutral-800 pt-6 list-none p-0">
              <li>✔️ Predicciones públicas básicas</li>
              <li>❌ Sin acceso a Bitácora Personal</li>
              <li>❌ Sin análisis del Cerebro IA</li>
            </ul>
          </div>
          <button onClick={onOpenAuth} className="w-full py-3 rounded-xl bg-neutral-800 hover:bg-neutral-750 font-bold text-sm uppercase transition-all cursor-pointer border-0 text-white">Comenzar</button>
        </div>
        <div className="bg-neutral-900 border-2 border-yellow-500 p-8 rounded-2xl space-y-6 flex flex-col justify-between relative shadow-[0_4px_30px_rgba(234,179,8,0.05)]">
          ...
          <div>
            ...
            <ul className="text-sm text-neutral-300 text-left space-y-3 mt-6 border-t border-neutral-800 pt-6 list-none p-0">
              <li>✔️ Acceso total a Vortex Analytics</li>
              <li>✔️ Bitácora My Tracker ilimitada</li>
              <li>✔️ Recomendaciones del Cerebro IA</li>
            </ul>
          </div>
          <button className="w-full py-3 rounded-xl bg-gradient-to-r from-red-600 to-yellow-500 text-neutral-950 font-black text-sm uppercase shadow-lg shadow-red-500/20 hover:scale-[1.02] transition-all cursor-pointer border-0">Adquirir VIP</button>
        </div>
      </div>
    </div>
  );
...
function TipsterPanel() {
  return <div className="p-8 text-center text-neutral-400 text-sm uppercase tracking-wider min-h-screen font-sans">💼 Panel de Tipster — Operativo en Producción</div>;
}

function AdminPanel() {
  return <div className="p-8 text-center text-neutral-400 text-sm uppercase tracking-wider min-h-screen font-sans">⚙️ Panel de Control Administrativo — Operativo en Producción</div>;
}

function LeaderboardPage() {
  return <div className="p-8 text-center text-neutral-400 text-sm uppercase tracking-wider min-h-screen font-sans">🏆 Ranking de Líderes — Operativo en Producción</div>;
}

function PaymentSuccess() {
  return <div className="min-h-screen bg-neutral-950 text-white flex items-center justify-center p-6 text-center font-sans"><div><span className="text-5xl mb-4 inline-block">🟢</span><h2 className="text-2xl font-black uppercase">Pago Procesado con Éxito</h2><p className="text-neutral-400 text-xs mt-2">Tu membresía VIP se ha activado en la base de datos de Aiven.</p></div></div>;
}
    ...
    <div className="fixed inset-0 bg-black/85 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-fadeIn font-sans">
      <div className="bg-neutral-900 border border-neutral-800 p-6 rounded-2xl w-full max-w-sm space-y-4 shadow-2xl">
        ...
        <div className="space-y-2 pb-4 border-b border-neutral-800">
          ...
          <button onClick={() => handleQuickLogin('free')} className="w-full py-2 rounded-xl bg-neutral-850 hover:bg-neutral-800 border border-neutral-800 text-xs font-bold uppercase tracking-wide text-neutral-400 transition-all cursor-pointer">Entrar como Gratuito (Free)</button>
          <button onClick={() => handleQuickLogin('premium')} className="w-full py-2 rounded-xl bg-yellow-500/10 hover:bg-yellow-500/20 border border-yellow-500/20 text-xs font-bold uppercase tracking-wide text-yellow-500 transition-all cursor-pointer">Entrar como Suscriptor (VIP)</button>
          <button onClick={() => handleQuickLogin('admin')} className="w-full py-2 rounded-xl bg-gradient-to-r from-red-600 to-yellow-500 text-neutral-950 text-xs font-black uppercase tracking-wide transition-all hover:scale-[1.01] cursor-pointer border-0">Entrar como Admin</button>
        </div>
        ...
        <button onClick={onClose} className="w-full py-3 rounded-xl bg-neutral-800 hover:bg-neutral-750 text-white font-bold text-sm uppercase cursor-pointer border-0">Entrar</button>
        <button onClick={onClose} className="w-full text-xs text-neutral-500 font-bold uppercase tracking-wider hover:text-neutral-400 transition-all cursor-pointer bg-transparent border-0">Cancelar</button>
      </div>
    </div>
  );
}
